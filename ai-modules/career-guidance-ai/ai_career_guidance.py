...
class AICareerGuidance:
    ...
    def get_job_market_data(self, domain: str, location: str = "United States") -> Dict:
        ...
        try:
            # Create cache key
            cache_key = f"{domain.lower()}_{location.lower()}"
            
            # Check cache first
            if cache_key in self._market_data_cache:
                print(f"🔄 Using cached market data for {domain} in {location}")
            ...
            if UTILS_AVAILABLE:
                ...
                result = self._get_fallback_market_data(domain, location)
    ...
    def generate_recommendations(self, user_profile: Dict, job_market_data: Dict) -> Dict:
        ...
        try:
            # Create cache key based on domain and experience
            domain = user_profile.get('domain_interest', '').lower()
            experience = user_profile.get('experience_years', 0)
            cache_key = f"{domain}_{experience}"
            
            # Check cache first
            if cache_key in self._recommendations_cache:
                print(f"🔄 Using cached recommendations for {domain} with {experience} years experience")
            ...
            if UTILS_AVAILABLE:
                ...
                result = self._get_fallback_recommendations(user_profile, job_market_data)
                # Cache fallback data too
                self._recommendations_cache[cache_key] = result
                return result
    ...
    def get_complete_analysis(self, 
        ...
        # Create cache key for complete analysis
        cache_key = f"complete_{domain_interest.lower().strip()}"
        
        # Check if we have a recent analysis for this domain
        if cache_key in self._analysis_cache:
            print(f"🔄 Using cached complete analysis for {domain_interest}")
        ...
        # Store for future reference and cache
        self.last_analysis = complete_analysis
        self._analysis_cache[cache_key] = complete_analysis
