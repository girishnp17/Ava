...
export default function LearningRoadmap() {
  ...
  const [createRoadmap, { data: roadmapResult, isLoading }] = useCreateRoadmapMutation();
  ...
  const getRoadmapData = () => {
    ...
    // The simple roadmap generator returns data directly in data.roadmap
    const roadmapData = data.roadmap || data;
    
    // Transform the simple roadmap structure to match expected format
    if (roadmapData && roadmapData.steps) {
      return {
        ...roadmapData,
        steps: roadmapData.steps.map((step: any, index: number) => ({
          step_number: index + 1,
          title: step.topic || step.title || `Step ${index + 1}`,
          description: step.description || '',
          duration: `Week ${step.week || index + 1}`,
          skills_to_learn: [],
          key_topics: [],
          learning_objectives: [step.description || ''],
          difficulty_level: data.skill_level || 'beginner'
        }))
      };
    }
  ...
  const { coursesByStep } = getCoursesData();
              ...
              <>
                ...
                <Paper sx={{ p: 3, mb: 4 }}>
                  <Typography variant="h5" sx={{ mb: 3, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    {parsedRoadmapData?.title || responseData?.roadmap?.title || `${responseData?.domain || formData.subject} Learning Roadmap`}
                  </Typography>
                  ...
                  <Grid container spacing={2} sx={{ mb: 3 }}>
                    <Grid item xs={12} sm={6} md={3}>
                      <Card sx={{ p: 2, textAlign: 'center' }}>
                        <Typography variant="h6" color="primary.main">{parsedRoadmapData?.steps?.length || responseData?.roadmap?.steps?.length || 8}</Typography>
                        <Typography variant="body2">Learning Steps</Typography>
                      </Card>
                    </Grid>
                    <Grid item xs={12} sm={6} md={3}>
                      <Card sx={{ p: 2, textAlign: 'center' }}>
                        <Typography variant="h6" color="success.main">{responseData?.duration || parsedRoadmapData?.total_duration || '8-12 weeks'}</Typography>
                        <Typography variant="body2">Total Duration</Typography>
                      </Card>
                    </Grid>
                    <Grid item xs={12} sm={6} md={3}>
                      <Card sx={{ p: 2, textAlign: 'center' }}>
                        <Typography variant="h6" color="info.main">{responseData?.skill_level || 'Beginner'}</Typography>
                ...
                <Paper sx={{ p: 3, mb: 4 }}>
                  ...
                  {parsedRoadmapData?.steps && (
                    <Stepper orientation="vertical">
                      {parsedRoadmapData.steps.map((step: RoadmapStep, index: number) => (
                              ...
                              <CardContent>
                                ...
                                {step.skills_to_learn && (
                                  <>
                                    ...
                                    <Box sx={{ mb: 3 }}>
                                      {step.skills_to_learn.map((skill: string, idx: number) => (
                                        <Chip
                                          key={idx}
                                          label={skill}
                                          color="primary"
                                ...
                                {step.key_topics && (
                                  <>
                                    ...
                                    <Box sx={{ mb: 3 }}>
                                      {step.key_topics.map((topic: string, idx: number) => (
                                        <Chip
                                          key={idx}
                                          label={topic}
                                          color="secondary"
                                ...
                                {step.learning_objectives && (
                                  <>
                                    ...
                                    <Box component="ul" sx={{ mb: 3, pl: 2 }}>
                                      {step.learning_objectives.map((objective: string, idx: number) => (
                                ...
                                {coursesByStep[`step_${step.step_number}`] && coursesByStep[`step_${step.step_number}`].length > 0 && (
                                  <>
                                    ...
                                    <Grid container spacing={2} sx={{ mb: 3 }}>
                                      {coursesByStep[`step_${step.step_number}`].slice(0, 3).map((course: CourseInterface, courseIdx: number) => (
                ...
                {courseRecommendations && courseRecommendations.length > 0 && (
                  <Paper sx={{ p: 3, mb: 4 }}>
                    ...
                    <Grid container spacing={3}>
                      {courseRecommendations.map((course: CourseInterface, index: number) => 
                ...
                {parsedRoadmapData?.career_outcomes && parsedRoadmapData.career_outcomes.length > 0 && (
                  <Paper sx={{ p: 3, mb: 4 }}>
                    ...
                    <Grid container spacing={1}>
                      {parsedRoadmapData.career_outcomes.map((outcome: string, index: number) => (
                        <Grid item key={index}>
                          <Chip
                            label={outcome}
                            color="success"
